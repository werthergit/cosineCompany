...
def merge(a:list,listAllTemp:list):
    ...
    for b in listAllTemp:
        if list(set(a).intersection(set(b))):
           #print("union",list(set(a).union(set(b))))
           list_group = list(set(a).union(set(b)))
    return list_group
...
if '__main__' == __name__:
    ...
    try:
        ...
        for row in results:
            ...
            #       (cid, cname, hid, hname))
            listCompany.append(cid)
        #print('listCompany:',listCompany)
        ...
        for listAll_1 in listAll:
            aaa = merge(listAll_1,listAllTemp)
            listResult.append(aaa)
